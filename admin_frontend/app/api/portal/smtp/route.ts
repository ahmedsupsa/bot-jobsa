...
export async function POST(req: Request) {
  try {
    ...
    if (!encKey) {
      return NextResponse.json({ error: "خطأ في الإعدادات: مفتاح التشفير غير موجود" }, { status: 500 });
    }

    let encrypted: string;
    try {
      encrypted = encryptAES(app_password, encKey);
    } catch (encErr: any) {
      return NextResponse.json({ error: "خطأ في التشفير: " + (encErr?.message || String(encErr)) }, { status: 500 });
    }
