...
import { CheckCircle2, Loader2, XCircle, Briefcase } from "lucide-react";
...
function SuccessContent() {
  ...
  useEffect(() => {
    ...
    fetch("/api/store/verify", {
      ...
      .then(j => {
        if (j.ok) setState("ok");
        else { setState("err"); setMsg(j.error || "فشل التحقق من الدفع"); }
      })
        ...
        {state === "ok" && (
          <>
            ...
            <p style={{ color: "#666", fontSize: 15, lineHeight: 1.8, margin: "0 0 32px" }}>
              تم تفعيل اشتراكك — سجّل الدخول لبوابة المستخدمين وارفع سيرتك الذاتية للبدء.
            </p>
